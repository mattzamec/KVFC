...
if (!$_GET['content'])
{
    $_GET['content'] = 'members';
}

// Get the membership_types array and prepare the membership_types table update form (in case needed)
$sql = @mysql_query('SELECT * FROM '.TABLE_MEMBERSHIP_TYPES, $connection) or die("Couldn't execute query 6.");
$membership_types_array = [];
$membership_type_ids = [];
...
while ( $row = mysql_fetch_object($sql) )
{
    ...
    else     // if order cost type is not explicitly set to percent, default it to fixed
    {
        $order_cost_display = '$&nbsp;'.number_format ($row->initial_cost, 2);
        $form_order_cost_unit = '$&nbsp;';
...
if ($_GET['content'] == 'members' && $_POST['update'] == 'Make changes')
{
    foreach (array_keys ($_POST['membership_type_id']) as $member_id)
    {
        if ($_POST['membership_type_id'][$member_id] != $_POST['old_membership_type_id'][$member_id])
        {
            $query = '
              UPDATE '.TABLE_MEMBER.'
              SET membership_type_id = "'.mysql_real_escape_string ($_POST['membership_type_id'][$member_id]).'"
...
{
    $page = ($_GET['page'] > 1 ? floor($_GET['page']) : 1);
    $sort = $_GET['sort'];
    ...
    $sort_by[$sort] = '&nbsp;(&#8593;)';
    $query = '
      SELECT COUNT(member_id) AS number_of_members
      FROM '.TABLE_MEMBER;
