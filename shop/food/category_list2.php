...
///                                                                          ///
/// &show_bulk=0|1           1 = show bulk categories; 0/null = regular      ///
///                                                                          ///
////////////////////////////////////////////////////////////////////////////////
...
// MZ: this is redundant for KVFC, but harmless
...
$_GET['depth'] = $_GET['depth'] ? floor($_GET['depth']) : 100;

//Make sure offset is numeric
$_GET['offset'] = floor($_GET['offset']);

//Set up the root for the categories tree
$base_category = $_GET['category_id'] ? preg_replace("/[^0-9]/","",$_GET['category_id']) : 0;
    ...
    function listCategories ($parent_id, $level)
    {
        ...
        { //There are more categories (or subcategories) to look at
            ...
            while ($row = mysql_fetch_array($sql))
            {
                ...
                { //Prepare output formatting
                    $item_count_markup = $subtotal.'&nbsp;item'.($subtotal > 1 ? 's' : '');
                    $item_new_count_markup = $subtotal_new > 0 ? '('.$subtotal_new.'&nbsp;new)' : '';
                    ...
                    {
                        $list_markup .= '<li>
                          <a href="category_list2.php?category_id='.$category_id.'&offset='.$level.'&show_bulk='.($show_bulk ? '1' : '0').'">'.$category_name.'</a>'.$classB_divider.'<span class="levelY">
                          <a href="product_list.php?type=full&category_id='.$category_id.'&show_bulk='.($show_bulk ? '1' : '0').'">'.$item_count_markup.'</a>
                          <a href="product_list.php?type=new&category_id='.$category_id.'&show_bulk='.($show_bulk ? '1' : '0').'">'.$item_new_count_markup."</a>
                          </span>\n";
        ...
        { //There are no more "categories", so call the subcategories and see how many items are in each
            $query2 ='
              ...
              WHERE '.TABLE_SUBCATEGORY.'.category_id = "'.$parent_id.'"'.
                $where_producer_pending.
                $where_auth_type.
                $where_unlisted_producer.
                $where_confirmed.
                $where_zero_inventory.'
              GROUP BY '.NEW_TABLE_PRODUCTS.'.subcategory_id
            ...
            while ($row2 = mysql_fetch_array($sql2))
            {
                ...
                { //Prepare output formatting
                    ...
                    $list_markup .= '<li>
                      <a href="product_list.php?type=full&subcat_id='.$subcategory_id.'&show_bulk='.($show_bulk ? '1' : '0').'">'.$subcategory_name.'</a>'.$classC_divider.'<span class="levelZ">
                      <a href="product_list.php?type=full&subcat_id='.$subcategory_id.'&show_bulk='.($show_bulk ? '1' : '0').'">'.$item_count_markup.'</a>
                      <a href="product_list.php?type=new&subcat_id='.$subcategory_id.'&show_bulk='.($show_bulk ? '1' : '0').'">'.$item_new_count_markup."</a>
                      </span></li>\n";
                }
            }
            if ($level <= $_GET['depth'])
...
$query = '
  SELECT *
  FROM '.TABLE_CATEGORY.'
  WHERE category_id = '.$base_category;
...
else
{
    ...
    $overall_category_name = '<h3 class="inline"><a href="'.$_SERVER['SCRIPT_NAME'].'?category_id='.$overall_parent_id.'&offset='.($_GET['offset'] - 1).'&show_bulk='.($show_bulk ? '1' : '0').'"><strong>&laquo;'.$row['category_name'].'</strong></a></h3>';
    if($_GET['show_parts'] == 'list_only')
    {
        $overall_category_name = $row['category_name'];
    }
...
$item_count_markup = $total.'&nbsp;item'.($total > 1 ? 's' : '');
$item_new_count_markup = $total_new > 0 ? '('.$total_new.'&nbsp;new)' : '';

$list_markup = $overall_category_name.'
  <div class="category_list2">
    <span class="levelX">
      <a href="product_list.php?type=full&category_id='.$base_category.'&show_bulk='.($show_bulk ? '1' : '0').'">'.$item_count_markup.'</a>&nbsp;&nbsp;
      <a href="product_list.php?type=new&category_id='.$base_category.'&show_bulk='.($show_bulk ? '1' : '0').'">'.$item_new_count_markup.'</a>
    </span>
    '.$sublist_markup.'
  </div>';
...
if($_GET['show_parts'] != 'list_only' && $_GET['show_parts'] != 'style_only')
{
