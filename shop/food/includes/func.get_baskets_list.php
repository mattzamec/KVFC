...
function get_baskets_list ($prm_bulk = false)
{
    global $connection;
    ...
    $query = '
      ...
      WHERE delivery_date > "'.mysql_real_escape_string($_SESSION['renewal_info']['membership_date']).'"
      AND date_open < NOW()
      AND is_bulk = '.($prm_bulk ? '1' : '0').'
    ...
    $query = '
      ...
      AND '.TABLE_ORDER_CYCLES.'.is_bulk = '.($prm_bulk ? '1' : '0').'
    ...
    $display .= '
        <div id="basket_dropdown" class="dropdown">
          <a href="'.$_SERVER['SCRIPT_NAME'].'?action=basket_list_only"><h1 class="basket_history">
            '.($prm_bulk ? 'Bulk ' : '').'Ordering History
          </h1></a>
        ...
        if ($after_current_count <= 2) // Including current
        {
            ...
            $display .= '
              <li class="'.$fe.$cg.$is.($full_empty == 'full' || $current == 'true' ? ' view' : '').'"'.($open_closed == 'open' ? ' id="current"' : '').'>
                ...
                ((new CurrentBasket())->basket_id() > 0 && $current == 'true' ? '
...
// Readable override of get_baskets_list(true)
function get_bulk_baskets_list()
{
    return get_baskets_list(true);
}