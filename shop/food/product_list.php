  ...
  function(data) {
    ...
    if (new_quantity > 0 && document.getElementById("add" + product_id)) // The item is in the basket
    ...
    else if (document.getElementById("add"+product_id) || document.getElementById("sub"+product_id)) // The item is not in the basket
      {
        ...
        if (elem = document.getElementById("message_area"+product_id)) elem.style.display = "none";
      }
    if (checked_out == 1) {
    ...
    if ($("#header_basket_quantity").length) {
      var headerBasketInfo = $("#header_basket_quantity").html();
      ...
      $("#header_basket_quantity").html(headerBasketAmount + " item" + (headerBasketAmount == 1 ? "" : "s")); 
    }
