...
// MZ: Displays the proper listed symbol and functionality
function listed_status_cell($data) {
    $listed_status = '
              <td class="product_list">
                <div id="list'.$data['product_id'].'">';

    if ($data['listing_auth_type'] == 'unlisted') {
        $listed_status .= '
                    <input type="image" class="list_check" src="'.DIR_GRAPHICS.'checkmark_x.png" onclick="ListProduct('.$data['product_id'].'); return false;">
                    <span class="conf_status0">Unlisted</span><br/>
                    <a class="basket_control" href="#" onclick="ListProduct('.$data['product_id'].'); return false;">List for retail</a>';
    }
    elseif ($data['listing_auth_type'] == 'member') {
        $listed_status .= '
                    <input type="image" class="list_check" src="'.DIR_GRAPHICS.'checkmark.png" onclick="UnlistProduct('.$data['product_id'].'); return false;">
                    <span class="conf_status1">Listed for retail</span><br/>
                    <a class="basket_control" href="#" onclick="UnlistProduct('.$data['product_id'].'); return false;">Unlist product</a>';
    }
    elseif ($data['listing_auth_type'] == 'archived') {
        $listed_status .= '
                    <img class="list_check" src="'.DIR_GRAPHICS.'checkmark_x_grey.png">
                    <span class="conf_status0">Archived</span><br/>
                    <a class="basket_control" href="edit_products.php?product_id='.$data['product_id'].'&product_version='.$data['product_version'].'&producer_id='.$_SESSION['producer_id_you'].'&a='.$_REQUEST['a'].'">Edit&nbsp;Product</a>';
    }
