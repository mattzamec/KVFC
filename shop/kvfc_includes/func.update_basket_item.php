...
function update_basket_item (array $data)
{
    ...
    {
        return ('Product ID = '.$data['product_id'].'; version = '.$data['product_version'].'; PVID = '.$data['pvid'].' not found in database.');
    }

//    $member_id_you = $_SESSION['member_id'];   // SHOULD THIS BE RESTORED?
    ...
    switch ($data['action'])
      {
        ...
        case 'producer_synch_ledger': // Used when checking_out
          ...
          break;
        default:
          return ('Unexpected request '.$data['action']);
    }
    ...
    {
        return ('Basket does not exist for member ID '.$data['member_id'].' and delivery ID '.$data['delivery_id']);
    }
    // Check for basket item
    if ($test_basket_item_exists && !is_array($basket_item_info))
    {
        return ('Basket item does not exist for basket ID '.$basket_info['basket_id'].', product ID '.$data['product_id'].' and product version'.$data['product_version']);
    }
    // Check if the basket is locked
    if ($basket_info['locked'] == 1)
    {
        return ('Basket ID '.$basket_info['basket_id'].' is locked');
    }
    ...
    if ($test_for_producer_privilege && $admin_override_not_set)
    {
        ...
        {
            return ('Producer is restricted from selling because they are unlisted or pending');
        }
    }
    // Check if the customer is allowed to purchase this product
    // MZ: This is redundant since all KVFC products' listing_auth_type is 'member', but leaving since it won't harm anything
    ...
    if ($test_for_membership_privilege && $admin_override_not_set
        && ($member_info['pending'] == 1 || $member_info['membership_discontinued'] == 1))
    {
        return ('Incorrect member privilege to order; member is pending or discontinued');
    }
    // Check if shopping is closed for this order
    $active_cycle = new ActiveCycle();
    if ($test_customer_ordering_window && $admin_override_not_set && !$active_cycle->is_open_for_ordering())
    {
        return ('Ordering is currently closed');
    }
    // Check if the product can be delivered to this site_id
    // MZ: This is redundant for KVFC, but harmless, so leaving as is.
    ...
    if ($test_producer_update_window && $producer_id_you && $admin_override_not_set && !$active_cycle->is_open_for_fulfillment())
    {
    ...
    if ($initiate_basket_item && !is_array($basket_item_info))
    {
        $query = '
          ...
          WHERE product_id = "'.mysql_real_escape_string ($product_info['product_id']).'"
        ...
        $basket_item_info = get_basket_item($basket_info['basket_id'], $product_info['product_id'], $product_info['product_version']);
    }
    ...
    if ($initiate_change_quantity)
    {
        ...
        {
            if ($matches[1] == '+') {
                $data['quantity'] = $matches[2]; // positive
            }
            elseif ($matches[1] == '-') {
                $data['quantity'] = 0 - $matches[2]; // negative
            }
            else {
                return ('Unexpected result 789830: expected a string starting with + or -');
            }
        }
        ...
        {
            return ('Unexpected result 785932: expected a string starting with + or - or containing only digits');
        }
        // If this is an inventory-controlled item
        // TODO: Aiveo issue #21 is likely in here 
    ...
    if ($initiate_change_outs)
    {
        ...
        if (preg_match('/^([\+\-])(\d+)$/', $data['out_of_stock'], $matches))
        {
            ...
            {
                return ('Unexpected result 578932: expected a string starting with + or -');
            }
        }
        ...
        {
            return ('Unexpected result 874042: expected a number or a string starting with + or -');
        }
...
//               }
    ...
    if ($basket_info['checked_out'] != 0)
    {
        ...
        if ($product_info['unit_price'] != 0)
        {
            ...
            {
                return ('Unexpected result 579210: random_weight is neither 0 nor 1');
            }
