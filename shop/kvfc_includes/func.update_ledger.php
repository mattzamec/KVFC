...
function search_ledger(array $data)
{
    ...
    elseif ($num_rows == 1)
    {
        ...
        $query_messages = '
          ...
          LEFT JOIN '.NEW_TABLE_MESSAGES.' USING(message_type_id)
          WHERE key1_target = "ledger.transaction_id"
...
function compare_ledger_fields ($data1, $data2)
{
    ...
    {
        $data2['target_key'] = get_internal_account_id($data2['target_key'], $data2['text_key']);
    }
    $difference_array = [];
    ...
    {
        if ($messages1[$message_key] != $messages2[$message_key])
        {
